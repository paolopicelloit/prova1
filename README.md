...
Here we explain how to use the **LTR Algorithms Execution** module to perform re-ranking starting from the files generated by the **Features Extraction** module execution. The full working of the software is controlled by java properties file.
...
`runsFolder` Path to the folder that contains the runs for which we want to perform re-ranking (same as Features Extraction)
